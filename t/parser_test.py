import template
from template import config, parser, test


class ParserTest(test.TestCase):
  def testParser(self):
    p2 = parser.Parser({"START_TAG": r"\[\*",
                        "END_TAG": r"\*\]",
                        "ANYCASE": 1,
                        "PRE_CHOMP": 1,
                        "V1DOLLAR": 1})
    ...
    p3 = config.Config.parser(
      {"TAG_STYLE": "html", "POST_CHOMP": 1, "ANYCASE": 1, "INTERPOLATE": 1})
    p4 = config.Config.parser({"ANYCASE": 0})
    tt = (("tt1", template.Template({"ANYCASE": 1})),
          ("tt2", template.Template({"PARSER": p2})),
          ("tt3", template.Template({"PARSER": p3})),
          ("tt4", template.Template({"PARSER": p4})))
